    ...
    <div class="max-w-4xl mx-auto space-y-6">
      ...
      <ToolCard>
        <div class="flex items-center gap-2 mb-6">
          ...
          <h2 class="text-lg font-semibold text-foreground">BMI 计算器</h2>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-6">
          <div class="space-y-4">
            <!-- 性别选择 -->
            <div>
              <label class="block text-sm font-medium text-foreground mb-2">性别</label>
          ...
          <!-- 结果展示 -->
          <div v-if="bmiResult" class="p-6 bg-primary/5 rounded-xl">
            <div class="text-center mb-4">
              <div class="text-sm text-muted-foreground mb-1">您的 BMI 指数</div>
            ...
            <div class="mt-4">
              ...
              <div class="relative h-4 mt-1">
                <div
                  class="absolute w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-b-8 border-b-foreground transform -translate-x-1/2"
          ...
          <div v-else class="p-6 bg-muted/50 rounded-xl flex items-center justify-center">
      ...
      <ToolCard v-if="bmiResult">
        ...
        <div class="grid md:grid-cols-2 gap-4">
          <div class="p-4 bg-muted/50 rounded-xl">
            <div class="text-sm text-muted-foreground mb-1">标准体重</div>
            <div class="text-2xl font-bold text-foreground">{{ bmiResult.standardWeight }} kg</div>
          ...
          <div class="p-4 bg-muted/50 rounded-xl">
            <div class="text-sm text-muted-foreground mb-1">与标准体重相差</div>
            <div
        ...
        <div class="mt-4 p-4 rounded-xl" :style="{ backgroundColor: bmiResult.color + '15' }">
          <div class="flex items-start gap-3">
            ...
            <div>
              <div class="font-medium text-foreground mb-1">健康建议</div>
              <div class="text-sm text-muted-foreground">{{ bmiResult.advice }}</div>
            </div>
          </div>
        </div>
      </ToolCard>
      ...
      <!-- BMI 分类说明 -->
      <ToolCard>
        <h3 class="text-base font-semibold text-foreground mb-4">BMI 分类标准 (中国标准)</h3>
        <div class="space-y-3">
          <div
            v-for="cat in bmiCategories"
            :key="cat.label"
            class="flex items-center justify-between p-3 rounded-lg"
            :class="bmiResult?.category === cat.label ? 'bg-muted' : 'bg-muted/50'"
          >
            ...
            <span class="text-sm text-muted-foreground">{{ cat.desc }}</span>
          </div>
        </div>

        <div class="mt-4 p-4 bg-blue-500/10 rounded-xl flex items-start gap-3">
